...
func (s *LedgerService) ReconcileBalance(_ context.Context, accountNumber string) error {
	...
	if err := s.db.Model(&model.LedgerEntry{}).
	...
	// Fetch the account's stored balance.
	var acct model.Account
	if err := s.db.Where("account_number = ?", accountNumber).First(&acct).Error; err != nil {
		return fmt.Errorf("reconcile: failed to fetch account %s: %w", accountNumber, err)
	}

	if !acct.Balance.Equal(net) {
		return fmt.Errorf("reconcile mismatch for account %s: stored balance=%s, ledger net=%s (credits=%s, debits=%s)",
