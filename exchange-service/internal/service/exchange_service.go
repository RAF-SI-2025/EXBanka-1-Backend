...
func NewExchangeService(repo *repository.ExchangeRateRepository, db *gorm.DB, commissionRate, spread string, redisCache *cache.RedisCache) (*ExchangeService, error) {
	...
	return &ExchangeService{repo: repo, upserter: repo, db: db, cache: redisCache, commissionRate: cr, spread: sp}, nil
}
...
func NewExchangeServiceWithUpserter(
	...
	svc, err := NewExchangeService(repo, db, commissionRate, spread, nil)
	if err != nil {
		return nil, err
	}
...
// getCachedRate checks Redis first, falls back to DB, populates cache on miss.
...
func (s *ExchangeService) SyncRates(ctx context.Context, p provider.RateProvider) error {
	...
	if syncErr != nil {
		...
		// Invalidate all cached rates after successful sync
		if s.cache != nil {
