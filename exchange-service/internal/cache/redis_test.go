...
func TestRedisCache_Set_MarshalError(t *testing.T) {
	...
	bad := make(chan int)
	require.Error(t, c.Set(context.Background(), "key", bad, time.Minute))
}
...
func TestRedisCache_DeleteByPattern(t *testing.T) {
	...
	require.NoError(t, c.Set(ctx, "rate:EUR:RSD", "v", time.Minute))
	require.NoError(t, c.Set(ctx, "rate:USD:RSD", "v", time.Minute))
	require.NoError(t, c.Set(ctx, "other", "v", time.Minute))
	require.NoError(t, c.DeleteByPattern(ctx, "rate:*"))
	var got string
	assert.Error(t, c.Get(ctx, "rate:EUR:RSD", &got))
