...
// It writes the principal_type / principal_id keys that ResolveIdentity
// reads (post-Spec-C-Task-3 — the legacy system_type / user_id keys
// are no longer set by any middleware).
...
func TestResolveIdentity_NoPrincipalReturns401(t *testing.T) {
	gin.SetMode(gin.TestMode)
	r := gin.New()
	// Note: no setPrincipal — principal_type / principal_id are never set.
	r.GET("/x",
		middleware.ResolveIdentity(middleware.OwnerIsPrincipal),
		func(c *gin.Context) { c.Status(http.StatusOK) })
	w := httptest.NewRecorder()
