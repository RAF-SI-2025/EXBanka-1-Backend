...
func TestLoad_DefaultsWhenEnvUnset(t *testing.T) {
	// Unset every variable Load reads so the defaults branch is exercised.
	for _, k := range []string{
		"CREDIT_DB_HOST", "CREDIT_DB_PORT", "CREDIT_DB_USER", "CREDIT_DB_PASSWORD",
		"CREDIT_DB_NAME", "CREDIT_GRPC_ADDR", "KAFKA_BROKERS",
		"ACCOUNT_GRPC_ADDR", "CLIENT_GRPC_ADDR", "USER_GRPC_ADDR", "METRICS_PORT",
	} {
		t.Setenv(k, "")
	}
	...
	assert.Equal(t, "5438", cfg.DBPort)
	assert.Equal(t, "postgres", cfg.DBUser)
	assert.Equal(t, "postgres", cfg.DBPassword)
	assert.Equal(t, "creditdb", cfg.DBName)
	assert.Equal(t, ":50058", cfg.GRPCAddr)
	assert.Equal(t, "localhost:9092", cfg.KafkaBrokers)
	assert.Equal(t, "localhost:50055", cfg.AccountGRPCAddr)
...
func TestLoad_OverridesFromEnv(t *testing.T) {
	t.Setenv("CREDIT_DB_HOST", "h")
	t.Setenv("CREDIT_DB_PORT", "1234")
	t.Setenv("CREDIT_DB_USER", "u")
	t.Setenv("CREDIT_DB_PASSWORD", "p")
	t.Setenv("CREDIT_DB_NAME", "n")
	t.Setenv("CREDIT_GRPC_ADDR", ":1")
	t.Setenv("KAFKA_BROKERS", "k:1")
	t.Setenv("ACCOUNT_GRPC_ADDR", "a:1")
	...
	assert.Equal(t, "a:1", cfg.AccountGRPCAddr)
...
func TestDSN_IncludesAllFields(t *testing.T) {
	cfg := &Config{
		DBHost: "h", DBPort: "1234", DBUser: "u", DBPassword: "p", DBName: "n",
	}
	dsn := cfg.DSN()
...
func TestDSN_HonoursSSLModeOverride(t *testing.T) {
	t.Setenv("CREDIT_DB_SSLMODE", "require")
