...
> **KNOWN ISSUE (2026-05-30) — cross-bank exercise buyer-credit is broken.** A buyer-side (`direction=CREDIT`) contract stores `buyer_id` as the buyer's **account number** (pinned from `OtcOffer.BuyerAccountNumber` at accept so the money leg debits the exact account). But the exercise CREDIT branch calls `parseSellerOwner(contract.BuyerID)`, which expects a participant id (`client-<n>`/`bank`) — so it fails with `Internal` ("buyer_id … not parseable, cannot credit shares") on every cross-bank exercise. Consequences observed live: the seller side exercises correctly (shares delivered, reservation consumed, strike money settled buyer→seller), but the buyer's contract stays `active` and the buyer receives no holding. Compounding this, the sender-side inline commit path (`InitiateOutboundTxWithPostings`) `MarkAttempt`s on a `materialiseOptions` failure but then **unconditionally `MarkCommitted`s**, so the row is marked committed despite the buyer-credit failure (no retry; `CommitOutboundLocal` does not re-materialise option legs, so the cron can't recover it either). Fixing requires a data-model decision: resolve the account number → owner before crediting (and define how the buyer's bank represents a holding of a security it doesn't list). Not yet fixed — money movement is correct on both sides; only the buyer's share delivery + contract status are affected.
...
- **Exercise does not re-reserve seller shares.** The NEW_TX-time share hold (`ReserveSellerSharesForNewTx`) fires only for accept-intent option legs; an `intent="exercise"` DEBIT option leg skips the reserve (the shares were already held at accept and are consumed at COMMIT by `RecordOptionContract`), per `OptionDescription.Intent`'s contract — otherwise the exercise would orphan a second hold that permanently locks the shares.
