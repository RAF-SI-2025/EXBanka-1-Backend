...
func TestClient_ListAndGet(t *testing.T) {
	...
	email := helpers.RandomEmail()
	createResp, err := c.POST("/api/v3/clients", map[string]interface{}{
		"first_name":    "List",
		"last_name":     "Test",
		"date_of_birth": helpers.DateOfBirthUnix(),
		"email":         email,
	...
	// Get by ID
	resp, err = c.GET(fmt.Sprintf("/api/v3/clients/%d", int(clientID)))
	if err != nil {
		t.Fatalf("error: %v", err)
	}
	helpers.RequireStatus(t, resp, 200)
...
func TestClient_Update(t *testing.T) {
	...
	resp, err := c.PUT(fmt.Sprintf("/api/v3/clients/%d", int(clientID)), map[string]interface{}{
...
func TestClient_GetNonExistent(t *testing.T) {
	t.Parallel()
	c := loginAsAdmin(t)
	resp, err := c.GET("/api/v3/clients/999999")
	if err != nil {
		t.Fatalf("error: %v", err)
	}
