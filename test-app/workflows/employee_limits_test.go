...
func TestLimits_GetEmployeeLimits(t *testing.T) {
	t.Parallel()
	c := loginAsAdmin(t)
	resp, err := c.GET("/api/v3/employees/1/limits")
	if err != nil {
...
func TestLimits_SetEmployeeLimits(t *testing.T) {
	...
	resp, err := c.PUT(fmt.Sprintf("/api/v3/employees/%d/limits", int(empID)), map[string]interface{}{
	...
	getResp, err := c.GET(fmt.Sprintf("/api/v3/employees/%d/limits", int(empID)))
	if err != nil {
		t.Fatalf("error: %v", err)
	}
...
func TestLimits_ApplyTemplate(t *testing.T) {
	...
	resp, err := c.POST(fmt.Sprintf("/api/v3/employees/%d/limits/template", int(empID)), map[string]interface{}{
...
func TestLimits_ListTemplates(t *testing.T) {
	t.Parallel()
	c := loginAsAdmin(t)
	resp, err := c.GET("/api/v3/limits/templates")
	if err != nil {
		t.Fatalf("error: %v", err)
	}
	helpers.RequireStatus(t, resp, 200)
...
func TestLimits_CreateTemplate(t *testing.T) {
	t.Parallel()
	c := loginAsAdmin(t)
	resp, err := c.POST("/api/v3/limits/templates", map[string]interface{}{
